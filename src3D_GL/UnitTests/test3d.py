...
def myifft3D(fhat):
  ...
  ## First do the fourier transform
  f =  scipy.fftpack.ifft(scipy.fftpack.ifft(fhat,axis=2),axis=0)*N1*N3
  ## Now do the chebyshev transform 
  for i in range(0,N1):
    for k in range(0,N3):
      f[i,:,k] = inverseChebTransform(f[i,:,k])
  return f
...
def myfft3D_rfast(u):
  ...
  N2 = N2 - 1
  u = scipy.fftpack.fft(scipy.fftpack.fft(u,axis=0),axis=2)/(N1*N3)
  umod = zeros((N1,2*N2,N3),dtype='complex')
  umod[:,0:N2+1,:] = u[:,0:N2+1,:]
  umod[:,N2+1:2*N2,:] = fliplr(u)[:,1:-1,:] 
  
  w = zeros((N1,N2,N3),dtype='complex')
...
def myfft3D_fast(u):
  ...
  for i in range(0,N1):
    for k in range(0,N3): 
      uhat[i,-1,k] = sum(1./N2*(-1.)**linspace(0,N2,N2+1)*1./c[i,:,k]*u[i,:,k])
  return uhat
...
def myfft3D(f):
  def transform2Cheb(f):
    ...
    return uhat
  N1,N2,N3 = np.shape(f)
  fhat = np.zeros((N1,N2,N3),dtype='complex')
  ## First do the fourier transform
  fhat[:,:,:] =  scipy.fftpack.fft( scipy.fftpack.fft(f[:,:,:],axis=0) ,axis=2) / (N1 * N3) 
  ## Now do the chebyshev transform 
  for i in range(0,N1):
    for k in range(0,N3):
      fhat[i,:,k] = transform2Cheb(fhat[i,:,k])
  return fhat
...
u = zeros((N1,N2,N3))
u[:,:,:] = L1/2.*sin(pi*y)*cos(4*pi*x/L1)*sin(2*pi*z/L3) + (y**2/2. - 0.5)
